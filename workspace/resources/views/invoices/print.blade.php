...
<head>
    ...
    <title>{{ $invoice->number ?? 'مسودة فاتورة' }} — {{ config('workspace.company.name') }}</title>
...
<body class="bg-surface font-sans text-ink">
    <div class="no-print mx-auto flex max-w-3xl justify-between gap-2 px-4 pt-6">
        <a href="{{ route('invoices.show', $invoice) }}" class="text-sm text-brand-800 hover:underline">→ العودة للفاتورة</a>
        <button type="button" onclick="window.print()" class="rounded-lg bg-brand-800 px-4 py-2 text-sm font-semibold text-white">طباعة / حفظ PDF</button>
    </div>
