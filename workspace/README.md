...
**الخروج التلقائي** بعد 15 دقيقة دون نشاط (حركة الفأرة، اللمس، لوحة المفاتيح،
الكتابة)، وبعد 12 ساعة من الدخول مهما كان النشاط (`SESSION_IDLE_TIMEOUT`
و`SESSION_ABSOLUTE_TIMEOUT` في `config/session.php`). قبلها بدقيقتين تنبيه «هل
ما زلت هنا؟» (`x-idle-timeout` و`idleTimeout` في `app.js`)، ومن يكتب نصاً طويلاً
ترسل صفحته نبضة كل دقيقة على الأكثر فلا يضيع ما كتبه. يفرضها على الخادم
`EnforceSessionTimeouts` ويسجّلها في «السجلات والتدقيق». لا خيار «تذكرني»: كوكيه
كان يعيد الدخول بصمت بعد انتهاء الجلسة فيُبطل المهلة.
