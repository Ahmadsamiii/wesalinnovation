...
function smallTalkReply(string $raw): ?string {
    ...
    if ($short && preg_match('/^(طيب|تمام|اوكي|ok|زين|ممتاز|اوك|كويس)$/u', $t))
        return "تمام. نكمل في نفس الموضوع، ولا عندك سؤال جديد؟";
    if (preg_match('/من انت|انت مين|عرف بنفسك|وش انت|مين انت/u', $t))
        return "أنا وصال، مساعد ذكي عربي مخصص لخدمة الأشخاص ذوي الإعاقة في المملكة.\nأجاوبك عن حقوقك، والخدمات والدعم المتاح لك، والتقنيات المساعدة، بمعلومات من مصادر سعودية رسمية.\nوش تحب تعرف؟";
...
function chatCircuitBreaker(string $ip, int $cost): void {
    if (CHAT_DAILY_IP_LIMIT > 0 &&
        ...
        out(['ok' => false, 'limit' => true,
             'error' => 'تجاوزت الحد اليومي للأسئلة من هذا الاتصال. حاول مرة أخرى غداً.'], 429);
    }
    if (CHAT_DAILY_TOTAL_LIMIT > 0 &&
        hitCounter('chat_day', '*', windowDay(), $cost) > CHAT_DAILY_TOTAL_LIMIT) {
        out(['ok' => false, 'busy' => true,
             'error' => 'وصلت المنصة إلى حدها اليومي من الأسئلة. حاول بعد قليل، ونعتذر عن الانتظار.'], 503);
    }
}
    ...
    if ($u) {
        ...
        if ((int)$u['tokens'] < $cost) {
            ...
            out(['ok' => false, 'limit' => true, 'renew' => $next * 1000,
                 'error' => 'انتهى رصيد أسئلتك لهذه الفترة، ويتجدّد تلقائياً بعد قليل.']);
        }
        ...
        if ($guestUsed > GUEST_LIMIT) {
            out(['ok' => false, 'needAuth' => true,
                 'error' => 'انتهت أسئلتك التجريبية. أنشئ حساباً مجانياً لتكمل، ويتجدّد رصيدك كل ٦ ساعات.']);
        }
    }
...
- لا تستخدم الشرطة الطويلة (—) ولا القصيرة (–) للفصل بين الكلام، واستخدم بدلها الفاصلة أو النقطة أو النقطتين.
- ادخل في الإجابة مباشرة، دون عبارات افتتاحية مثل «سؤال رائع» أو «بالتأكيد!».
...
- لو حيّاك المستخدم أو سأل عن حالك أو شكرك، ردّ بتحية أو مجاملة عربية دافئة مختصرة ثم اعرض مساعدتك، ولا تتجاهل التحية أبداً.
